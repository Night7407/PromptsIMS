...
- Entity: User
    ...
    - role: Role enum (INVENTORY_MANAGER, ADMIN) — stored as STRING, not ORDINAL
...
- Endpoints under /api/products:
    ...
    - GET / -> list all
    - GET /{id} -> get one, throws ProductNotFoundException if missing
...
- ProductControllerIntegrationTest (@SpringBootTest RANDOM_PORT +
  Testcontainers PostgreSQL): verify that a request with X-User-Role=ADMIN
  header can create a product, a request with X-User-Role=INVENTORY_MANAGER
  gets 403 on POST but 200 on GET, and a request with no headers gets 401/403
  on every endpoint.
...
- Endpoints under /api/suppliers:
    ...
    - GET / -> list all (ADMIN, INVENTORY_MANAGER)
...
- SupplierControllerIntegrationTest (@SpringBootTest RANDOM_PORT +
  ...
  catalog-service's tests (ADMIN can mutate, INVENTORY_MANAGER can only
...
- Endpoints under /api/inventory:
    - POST /adjustments -> record a StockAdjustment (INVENTORY_MANAGER,
      ADMIN)
    - GET /adjustments/{productId} -> adjustment history for a product
    - GET /alerts?status=OPEN -> list alerts (INVENTORY_MANAGER, ADMIN)
    - POST /alerts/check/{productId} -> manually trigger
      checkAndCreateAlertIfNeeded (mainly for testing/demo; ADMIN only)
...
- Endpoints under /api/purchase-orders:
    - POST /draft -> creates a PurchaseOrder with status PENDING_APPROVAL.
      Accessible to ROLE_SERVICE, INVENTORY_MANAGER, or ADMIN.
    - PUT /{id}/approve -> sets status SENT, approvedBy (from X-User-Id
      header), approvedAt=now, expectedDeliveryDate. ADMIN only — and must
      explicitly reject any request authenticated as ROLE_SERVICE (return
      403) even if the internal token is valid, since approval must always
      be a real human manager action.
    - PUT /{id}/receive -> sets status RECEIVED. INVENTORY_MANAGER, ADMIN.
    - PUT /{id}/cancel -> sets status CANCELLED. ADMIN only.
    - GET / -> list all, optional ?status= filter. INVENTORY_MANAGER, ADMIN.
...
- PurchaseOrderControllerIntegrationTest (@SpringBootTest RANDOM_PORT +
  Testcontainers PostgreSQL):
    ...
    - X-User-Role=INVENTORY_MANAGER gets 403 on PUT /{id}/approve but 200 on
      PUT /{id}/receive
...
the frontend can call on behalf of a logged-in manager.
...
- Define Spring AI @Tool-annotated methods (or FunctionCallback beans,
  ...
  (passed in from the frontend, which stores them from the manager's
  session) — the chatbot must never have broader access than the manager
  asking the question.
...
using Thymeleaf, acting as a Backend-for-Frontend that authenticates
managers via session and calls backend services through the api-gateway.
...
- Pages/controllers (Thymeleaf templates, minimal styling is fine — focus on
  functionality):
    ...
    - ProductController: GET /products -> list, GET /products/new + POST
      /products -> create (ADMIN only — hide the "new" button in the
      template if session role isn't ADMIN)
    - SupplierController: GET /suppliers -> list
    - PurchaseOrderController: GET /purchase-orders -> list with an
      "Approve" button visible only for ADMIN role, POST
