...
**Core security model:** JWT is validated **once, at the Gateway**. Downstream services trust two headers the Gateway injects after validation (`X-User-Id`, `X-User-Role`), plus a shared `X-Internal-Service-Token` header for service-to-service calls. Identity is the exception only for `/auth/login` — it keeps `permitAll` there since login happens before a token exists. There is no public self-registration: `POST /api/users` (account creation) requires a real human JWT with role `ADMIN`, using the same header-trust model as every other protected endpoint in the system. Approval of purchase orders must always require a real human JWT (MANAGER or ADMIN) — `ROLE_SERVICE` is explicitly blocked from that one endpoint.
...
### Prompt 4 — Password validation, remove public registration, admin-managed user creation (run after Prompt 3)
```
ROLE: Act as a senior Java developer hardening the already-built
identity-service.

TASK: Replace open self-registration with admin-only user creation, and
add server-side password validation. Do not touch JwtUtil, login logic, or
the User entity's existing columns.

CONTEXT:
- Remove POST /auth/register entirely — there is no public registration in
  this system.
- Add POST /api/users, restricted to callers with role ADMIN only. Accepts
  the same fields registration used to (username, password, email, role),
  creates the User exactly as /auth/register used to, returns
  UserResponseDto with 201.
- Add password validation applied to this new endpoint's request DTO:
  minimum 8 characters, at least 1 uppercase letter, 1 lowercase letter, 1
  digit, 1 special character (!@#$%^&*()_+-=). Implement as a custom
  @ValidPassword Bean Validation annotation + ConstraintValidator, not
  inline if/else checks. Wire validation failures into the existing
  GlobalExceptionHandler as a 400 with field + message.
- Since this service now has a protected endpoint beyond /auth/login, add a
  HeaderAuthenticationFilter (plain OncePerRequestFilter) identical in
  shape to the one used in catalog-service/supplier-service: reads
  X-User-Id and X-User-Role headers (set by the Gateway after JWT
  validation) and populates the SecurityContext. Update SecurityConfig:
  permitAll only for /auth/login, everything else requires authentication,
  /api/users POST requires ADMIN via @PreAuthorize.
- Add a bootstrap seed: a CommandLineRunner that, on startup, checks if any
  User with role ADMIN exists; if none, inserts one default ADMIN user
  (hashed password) so the system is never in a state where no one can
  create accounts. Log the default credentials once at startup with a
  clear "CHANGE ME" warning — do not hardcode them in documentation.

OUTPUT FORMAT: Complete files only — updated AuthController (register
method removed), new UserAdminController, @ValidPassword +
PasswordConstraintValidator, updated CreateUserRequestDto (rename from
RegisterRequestDto if that's cleaner), HeaderAuthenticationFilter, updated
SecurityConfig, BootstrapAdminSeeder. No narrative explanation.
```

### Prompt 5 — Employee listing for MANAGER/ADMIN (run after Prompt 4)
```
ROLE: Act as a senior Java developer.

TASK: Add a read endpoint so MANAGER and ADMIN can list users, primarily to
power a frontend "Employees" screen.

CONTEXT:
- GET /api/users?role=EMPLOYEE — accessible to MANAGER and ADMIN only
  (@PreAuthorize). Optional role query param filters by EMPLOYEE or
  MANAGER; if role=ADMIN is requested, only an ADMIN caller may see it
  (MANAGER gets 403 for that specific filter value — check the caller's
  own role against the requested filter inside the service method, not
  just via @PreAuthorize).
- Paginate with Spring Data's Pageable, default page size 20.
- Response: Page<UserResponseDto> — id, username, email, role, createdAt
  (never return the password hash).

OUTPUT FORMAT: Complete files only — updated UserAdminController and
UserService method, plus a short UserServiceTest addition. No narrative
explanation.
```

### Prompt 6 — Tests for Prompts 4 & 5
```
ROLE: Act as a senior Java developer writing tests.

TASK: Generate tests covering the admin-user-management changes.

CONTEXT:
- Unit tests: PasswordConstraintValidatorTest (too short, missing
  uppercase, missing digit, missing special char, valid password all
  covered), UserServiceTest additions for createUser (ADMIN succeeds,
  non-ADMIN blocked at the controller/security layer) and listUsers
  (manager can list employees, manager blocked from listing admins, admin
  can list anyone).
- Integration test: UserAdminControllerIntegrationTest using
  @SpringBootTest(webEnvironment = RANDOM_PORT) + Testcontainers Postgres,
  simulating the X-User-Id/X-User-Role headers directly (since the Gateway
  isn't present in this test), covering create-user and list-users happy
  paths plus the 403 cases.

OUTPUT FORMAT: Complete test files with full imports. No prose, code only.
```
...
### Prompt 10.1 — Admin user creation & manager employee list (run after Prompt 10)
```
ROLE: Act as a senior Java developer extending the Thymeleaf BFF.

TASK: Add two new screens to the already-built web-app: an ADMIN-only
"Create User" page, and a MANAGER/ADMIN "Employees" list page. There is no
public registration page — the login page stays login-only.

CONTEXT:
- AdminUserController: GET /admin/users/new (renders form, ADMIN only —
  redirect non-admins to /dashboard), POST /admin/users (calls
  api-gateway's POST /api/users with the session JWT attached via the
  existing RestTemplate interceptor, role dropdown EMPLOYEE/MANAGER/ADMIN).
  On success, redirect to /admin/users with a flash success message; on
  validation failure (password policy, duplicate username), redisplay the
  form with field errors from the backend's error response.
- EmployeeController: GET /employees (MANAGER or ADMIN only — redirect
  EMPLOYEE role to /dashboard), calls api-gateway's
  GET /api/users?role=EMPLOYEE, renders a paginated table (username,
  email, role, createdAt).
- Templates: create-user.html and employees.html, reusing the shared
  layout fragment (header/nav) already established for the other pages.
- Nav bar update: add "Create User" link visible only when session role is
  ADMIN, and "Employees" link visible when session role is MANAGER or
  ADMIN — same visibility pattern already used for the "new product"
  button.

OUTPUT FORMAT: Complete files only — AdminUserController, EmployeeController,
create-user.html, employees.html, updated nav fragment. File-path comments
before each file. No prose outside code comments.
```
...
6. **After Prompt 4 (Identity), check the startup log for the seeded bootstrap ADMIN credentials** before trying to log in — there is no other way to get the first account, since registration is admin-only now.
